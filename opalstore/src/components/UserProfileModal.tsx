    ...
    <>
      ...
      <div style={{ position: 'fixed', inset: 0, zIndex: 70, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
        <div style={{ width: '100%', maxWidth: '400px', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.06)', overflow: 'hidden', background: '#141414' }} onClick={(e) => e.stopPropagation()}>
          ...
          {/* Header */}
          <div style={{ position: 'relative', height: '80px', background: '#d4a855' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', marginTop: '-40px', position: 'relative', zIndex: 10 }}>
            {avatarUrl ? (
              ...
              <div style={{ width: '84px', height: '84px', borderRadius: '50%', border: '4px solid #141414', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '28px', fontWeight: '700', color: '#fff', background: '#d4a855' }}>
          ...
          <div style={{ display: 'flex', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
            <button onClick={() => setActiveTab('profile')} style={{ flex: 1, padding: '10px', fontSize: '13px', fontWeight: '600', background: 'none', border: 'none', cursor: 'pointer', color: activeTab === 'profile' ? '#fff' : '#71717a', borderBottom: activeTab === 'profile' ? '2px solid #e0b870' : '2px solid transparent' }}>
              Profil
            </button>
            <button onClick={() => setActiveTab('password')} style={{ flex: 1, padding: '10px', fontSize: '13px', fontWeight: '600', background: 'none', border: 'none', cursor: 'pointer', color: activeTab === 'password' ? '#fff' : '#71717a', borderBottom: activeTab === 'password' ? '2px solid #e0b870' : '2px solid transparent' }}>
              Kata Sandi
            </button>
          </div>
          ...
          <div style={{ padding: '16px' }}>
            ...
            {activeTab === 'password' && (
              <form onSubmit={handlePasswordUpdate} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                ...
                <button type="submit" disabled={isLoading} style={{ width: '100%', padding: '10px', borderRadius: '10px', fontSize: '13px', fontWeight: '600', color: '#fff', background: '#d4a855', border: 'none', cursor: isLoading ? 'not-allowed' : 'pointer', opacity: isLoading ? 0.5 : 1 }}>
